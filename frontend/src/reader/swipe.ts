 ...
 * gesture feels the same on a phone and on a tablet. Clamped so a very narrow
 * (or unmeasured, e.g. jsdom) viewport still needs a deliberate movement. */
export function swipeThreshold(width: number): number {
  return Math.min(120, Math.max(40, width * 0.15));
}
